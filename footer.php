...
<footer class="footer flex flex-col md:flex-row mt-8 bg-lightBg print:hidden">
  <div class="container flex flex-col md:flex-row gap-6 m-auto py-6 px-4 md:px-6 print:hidden">
    ...
    <div class="w-full md:w-1/3 md:pl-4 border-b pb-4 md:border-none md:border-l border-border flex flex-col gap-4  md:text-right">
      ...
      ?>
      <img src="<?php echo esc_url($intro_settings['logo']); ?>" class="logo max-w-[250px]  md:mx-0 mt-4 md:mt-8" alt="<?php echo esc_attr(get_bloginfo('name')); ?>">
    ...
    <div class="w-full md:w-2/3 print:hidden">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mr-0 md:mr-4">
        ...
        if ($footer_menu_settings['menu_id']) {
          ...
          if ($menu_items) {
            // تقسیم آیتم‌ها به گروه‌های ۵ تایی
            $menu_chunks = array_chunk($menu_items, 5);

            foreach ($menu_chunks as $chunk_index => $chunk) {
              echo '<ul class="flex flex-col gap-2 list-disc list-inside text-right marker:text-lg marker:text-grayText marker:font-bold">';
